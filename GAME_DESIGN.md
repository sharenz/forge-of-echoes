...
Character art uses two complementary fidelity tiers. Class selection may use detailed authored roster art to establish face, armor, weapon, material, and palette identity. The simulated world uses compact deterministic pixel sprites with class-specific silhouettes and palettes, expanded idle/walk/attack/cast frame sets, and pooled supporting effects. A subtle aura, footstep dust, weapon sweep, projectile trail, impact spark, and skill core combine into the final look; no single layer should carry the entire visual effect or obscure enemies and drops.
