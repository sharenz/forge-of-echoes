...
Character art uses two complementary fidelity tiers. Class selection uses detailed authored roster art to establish face, armor, weapon, material, and palette identity. The simulated world uses optimized authored pixel sprites built specifically for the slightly elevated three-quarter camera, with one readable silhouette per class and a transparent logical movement anchor. Runtime pose animation adds breathing, stride, weight shift, attack recoil, cast anticipation, directional flipping, and dash squash without moving the combat coordinate. A subtle aura, footstep dust, weapon sweep, projectile trail, impact spark, and skill core combine into the final look; no single layer should carry the entire visual effect or obscure enemies and drops.
